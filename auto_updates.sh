...
EMAIL="ton@email.com"
LOG_DIR="/var/log"
THRESHOLD_SPACE=90  # Seuil d'espace disque critique en %
THRESHOLD_MEMORY=80  # Seuil de mémoire critique en %
...
if [ $? -eq 0 ]; then
    ...
    echo "Mises à jour terminées avec succès." | mail -s "Mises à jour système" $EMAIL
else
    echo "Échec de la mise à jour. Vérifiez les erreurs." | mail -s "Erreur mise à jour système" $EMAIL
fi

# 2. Vérifier l'espace disque
DISK_USAGE=$(df -h / | tail -n 1 | awk '{print $5}' | cut -d'%' -f1)
if [ $DISK_USAGE -ge $THRESHOLD_SPACE ]; then
    echo "Alerte : Espace disque critique ($DISK_USAGE%) sur /" | mail -s "Alerte espace disque" $EMAIL
fi

# 3. Vérifier l'utilisation de la mémoire
MEMORY_USAGE=$(free | grep "Mem:" | awk '{print $3/$2 * 100.0}')
if (( $(echo "$MEMORY_USAGE > $THRESHOLD_MEMORY" | bc) )); then
    echo "Alerte : Utilisation mémoire critique ($MEMORY_USAGE%)" | mail -s "Alerte mémoire" $EMAIL
fi
...
echo "Nettoyage des logs terminé." | mail -s "Nettoyage logs" $EMAIL
